...
def labels_count(annotations, nclasses, invalid=MISSING_VALUE):
...
def majority_vote(annotations, invalid=MISSING_VALUE):
...
def labels_frequency(annotations, nclasses):
    ...
    """
    counts=np.array(labels_count(annotations, nclasses))
    sum_counts=float(np.sum(counts))
    freq=list(counts/sum_counts)
    return freq
